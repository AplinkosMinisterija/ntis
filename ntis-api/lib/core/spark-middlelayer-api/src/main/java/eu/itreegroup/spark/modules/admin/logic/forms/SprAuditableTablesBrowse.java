...
public class SprAuditableTablesBrowse extends FormBase {
	...
	public SprAuditableTablesBrowse(BaseControllerJDBC queryController, DBStatementManager dbstatementManager,
			DBStatementManager dbStatementManager2, SprAuditableTablesDBService sprAuditableTablesDBService) {
		super();
	...
	public String getAuditableTablesList(Connection conn, SelectRequestParams params, Double userId) throws Exception {
		...
		this.managePredefinedFilterStructure(conn, advancedParamList.get(PREDEFINED_FILTER_PARAM), userId, stmt,
				advancedParamList);
		stmt.addParam4WherePart("aut_table_name", StatementAndParams.PARAM_STRING,
				advancedParamList.get("aut_table_name"));
		stmt.addParam4WherePart("aut_table_schema", StatementAndParams.PARAM_STRING,
				advancedParamList.get("aut_table_schema"));
		stmt.addParam4WherePart("aut_id", StatementAndParams.PARAM_DOUBLE, advancedParamList.get("aut_id"));
		stmt.addParam4WherePart(
				dbstatementManager.colNamesToConcatString("aut_table_name", "aut_table_schema", "aut_id"),
	...
	public String getAuditableTablesRecList(Connection conn, SelectRequestParams params, Double userId)
			throws Exception {
		log.debug("Start extract information from db");
		this.checkIsFormActionAssigned(conn, SprAuditableTablesBrowse.ACTION_READ);
		...
		SprAuditableTablesDAO tab = sprAuditableTablesDBService.loadRecord(conn,
				Double.valueOf(paramList.get("p_aut_id")));
		...
		stmt.addParam4WherePart("rec_aud_timestamp", StatementAndParams.PARAM_DATE_TIME,
				advancedParamList.get("rec_aud_timestamp"),
				dbStatementManager.getDateFormat(DBStatementManager.DATE_FORMAT_DAY_TIME_JAVA));
		stmt.addParam4WherePart("rec_userid", StatementAndParams.PARAM_STRING,
				advancedParamList.get("rec_aud_user_id"));
		if (tab.getAut_table_name().equals("ntis_building_ntrs")) {
			stmt.addParam4WherePart(
					dbStatementManager.colNamesToConcatString("REC_AUD_ID", "REC_AUD_OPERATION_TYPE",
							"aud_ntis_building_ntrs.bn_aob_code", "aud_ntis_building_ntrs.bn_obj_inv_code",
							"TO_CHAR(REC_AUD_TIMESTAMP, '"
	...
	public void setAuditEnabled(Connection conn, SwitchStatus params) throws SparkBusinessException, Exception {
		...
		if (DBStatementManager.DB_TYPE_POSTGRESQL.equalsIgnoreCase(dbStatementManager.getDbType())) {
			...
			stmt.addSelectParam(
					sprAuditableTableDAO.getAut_table_schema() + "." + sprAuditableTableDAO.getAut_table_name());
			...
			stmt.setStatement("ALTER TRIGGER  " + sprAuditableTableDAO.getAut_table_schema() + "."
					+ sprAuditableTableDAO.getAut_trigger_name() + " "
	...
	public SprAuditableTablesDAO save(Connection conn, SprAuditableTablesDAO record) throws Exception {
		this.checkIsFormActionAssigned(conn,
				record.getRecordIdentifier() == null ? SprFormsBrowse.ACTION_CREATE : SprFormsBrowse.ACTION_UPDATE);
	 ...
	 * @throws Exception
	 */
	public SprAuditableTablesDAO get(Connection conn, RecordIdentifier recordIdentifier)

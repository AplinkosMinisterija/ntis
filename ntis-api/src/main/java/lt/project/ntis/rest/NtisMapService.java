...
/**
 * Klasė skirta gauti/siųsti duomenis reikalingus žemėlapiui (prisijungusiam vartotojui)
 * 
 */

@RestController
@RequestMapping("/ntis-map")
@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
public class NtisMapService extends S2RestAuthService<SprBackendWebSessionInfo, SprBackendUserSession> {
    ...
    @GetMapping(value = "/get-counties-municipalities", produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    @PostMapping(value = "/get-agglo-table-geoms", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<List<NtisAggloMapTableGeom>> getAggloTableGeoms(@RequestBody(required = false) ArrayList<AdvancedSearchParameterStatement> filters)
            throws Exception {
        return okResponse(ntisMap.getAggloTableGeoms(this.getDBConnection(), filters));
    }
    ...
    @GetMapping(value = "/get-tiles/{layer}/{z}/{x}/{y}")
    public ResponseEntity<Resource> getTilesWithAuth(@RequestHeader HttpHeaders headers, @PathVariable(name = "layer") String layer,
            @PathVariable(name = "z") String z, @PathVariable(name = "x") String x, @PathVariable(name = "y") String y)
            throws SparkBusinessException, IOException {
        return this.ntisMap.getTilesWithAuth(this.getDBConnection(), headers, layer, z, x, y);
    }

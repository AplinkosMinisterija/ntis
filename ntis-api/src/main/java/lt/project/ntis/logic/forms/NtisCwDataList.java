...
/**
 * Klasė skirta formos "Centralizuoto nuotekų tvarkymo duomenys" (formos kodas
 * N4200) biznio logikai apibrėžti, kai naudotojas turi ntis administratoriaus
 * teises
 */
...
public class NtisCwDataList extends FormBase {
	...
	public void defineFormAndActions() {
		setFormInfo(getFormName(), "Centralizuoto nuotekų tvarkymo duomenys",
				"Centralizuoto nuotekų tvarkymo duomenų forma administratoriui");
		addFormActions(FormBase.ACTION_READ, FormBase.ACTION_CREATE, FormBase.ACTION_UPDATE, FormBase.ACTION_DELETE);
	}

	/**
	 * Metodas grąžins centralizuoto nuotekų tvarkymo duomenų formoje naudojamus
	 * pavyzdinius failus
	 * 
	...
	public List<SprFile> getExemplaryFiles(Connection conn) throws Exception {
		...
		methods.add(new MethodCaller("setFilKey4Encript",
				new StatementDataGetter[] {
						new JavaObjectGetter("fileStorageService", fileStorageService, FileStorageService.class),
						new StatementStringGetter("fil_key") }));
		...
		List<SprFile> data = queryController.selectQueryAsObjectArrayList(conn, stmt,
				new Data2ObjectProcessor<SprFile>(SprFile.class, methods));
		return data;
	}
	...
	public String getCwFilesList(Connection conn, SelectRequestParams params, Double orgId) throws Exception {
		...
		StatementAndParams stmt = new StatementAndParams();
		ArrayList<MethodCaller> methods = new ArrayList<MethodCaller>();
		methods.add(new MethodCaller("setFilKeyEncrypted",
				new StatementDataGetter[] {
						new JavaObjectGetter("fileStorageService", fileStorageService, FileStorageService.class),
						new StatementStringGetter("fil_key") }));
		stmt.setStatement("SELECT CW.CWF_ID, " + //
				"TO_CHAR(CW.CWF_IMPORT_DATE, '"
				+ dbstatementManager.getDateFormat(DBStatementManager.DATE_FORMAT_DAY_DB) + "') AS CWF_IMPORT_DATE, " + //
				"CW.CWF_STATUS, " + //
				"CW.CWF_FIL_ID, " + //
				"USR.USR_PERSON_NAME, " + //
				...
				"TO_CHAR(FIL.FIL_STATUS_DATE, '"
				+ dbstatementManager.getDateFormat(DBStatementManager.DATE_FORMAT_DAY_DB) + "') AS FIL_STATUS_DATE " + //
		...
		stmt.addParam4WherePart("cwf_import_date", StatementAndParams.PARAM_DATE,
				advancedParamList.get("cwf_import_date"),
	...
	/**
	 * Pagal per parametrus perduodama failo rakt, metodas sukurs naują
	 * centralizuoto nuotekų tvarkymo įrašą NTIS_CW_FILES lentelėje.
	 * 
	 * @param conn     - prisijungimas prie DB
	 ...
	 *
	 */
	public NtisCwFilesDAO saveNewFile(Connection conn, String fileKey, Double usrId, Double orgId, Languages language)
			throws Exception {
	...
	public void validateFileErrs(Connection conn, Double cwfId, Double vandOrgId, Double usrId, Double sesOrgId,
			Languages language) throws Exception {
		...
		NtisCwFilesDAO fileDAO = cwFilesDBService.loadRecordByParams(conn, "cwf_id = ?::int and cwf_org_id=?::int",
				new SelectParamValue(cwfId), new SelectParamValue(vandOrgId));
		...
		Double jobRequestId = ntisCentralDataValidationJobRequest.createJobRequestWithAuthors(conn, usrId, sesOrgId,
				language, params);
		conn.commit();
		executorJob.execute(jobRequestId);
		conn.commit();
	...
	/**
	 * Metodas ištrins centralizuoto nuotekų tvarkymo bylos failą ir su juo
	 * susijusius įrašus
	 * 
	 * @param conn - prisijungimas prie DB
	...
	public void deletePendingFile(Connection conn, SprFile file, Double orgId) throws Exception {
		...
		if (orgId != null) {
			if (cwFile.getCwf_org_id().compareTo(orgId) != 0) {
				throw new SparkBusinessException(new S2Message("common.error.action_not_granted",
						SparkMessageType.ERROR, "User is not authorized to perform this action"));
			}
		}
	...
	/**
	 * Pagal nurodytą įrašo id, metodas atnaujins importuotos bylos statusą į
	 * galutinį
	 * 
	 * @param conn  - prisijungimas prie DB
	...
	public void updateExistingFile(Connection conn, String id, Double orgId, Double usrId) throws Exception {
		...
		StatementAndParams stmt = new StatementAndParams(
				"CALL ntis.update_building_agreements(?::integer, ?::integer)");
	...
	public NtisWastewaterDataImportFile getFileRecord(Connection conn, Double cwfId, Double orgId) throws Exception {
		...
		List<NtisWastewaterDataImportFile> data = queryController.selectQueryAsObjectArrayList(conn, stmt,
				NtisWastewaterDataImportFile.class);
	...
	/**
	 * Metodas grąžins centralizuoto nuotekų tvarkymo bylos importo įraše nuskaitytų
	 * klaidų sąrašą
	 * 
	 * @param conn   - prisijungimas prie DB
	 * @param params - select parametrai
	 ...
	 * @throws Exception
	 */
	public String getAddressMappings(Connection conn, SelectRequestParams params, Double orgId, String lang)
			throws Exception {
		this.checkIsFormActionAssigned(conn, NtisCwDataList.ACTION_READ);
	 ...
	 * @throws Exception
	 */
	public NtisAdrMappingsDAO getAdrMapping(Connection conn, RecordIdentifier recordIdentifier, Double orgId)
			throws NumberFormatException, Exception {
		if ("new".equalsIgnoreCase(recordIdentifier.getId())
				|| "new".equalsIgnoreCase(recordIdentifier.getActionType())) {
			this.checkIsFormActionAssigned(conn, NtisCwDataList.ACTION_CREATE);
		} else {
			this.checkIsFormActionAssigned(conn, NtisCwDataList.ACTION_READ);
		}
	...
	public NtisAdrMappingsDAO saveAdrMapping(Connection conn, NtisAdrMappingsDAO record, Double orgId)
			throws Exception {
		this.checkIsFormActionAssigned(conn,
				record.getRecordIdentifier() == null ? NtisCwDataList.ACTION_CREATE : NtisCwDataList.ACTION_UPDATE);
	...
	public void setFileForErrProcessing(Connection conn, Double id, Double orgId) throws Exception {
		...
		if (orgId != null) {
			NtisCwFilesDAO fileDAO = cwFilesDBService.loadRecordByParams(conn, "cwf_id = ?::int and cwf_org_id=?::int",
					new SelectParamValue(id), new SelectParamValue(orgId));
			if (fileDAO == null || fileDAO.getCwf_id() == null) {
				throw new SparkBusinessException(new S2Message("common.error.action_not_granted",
						SparkMessageType.ERROR, "User is not authorized to perform this action"));
			}
	...
	public void revalidateErrors(Connection conn, Double id, Double vandOrgId, Double usrId, Double sesOrgId,
			Languages language) throws NumberFormatException, Exception {
		this.checkIsFormActionAssigned(conn, NtisCwDataList.ACTION_UPDATE);
		NtisCwFilesDAO fileDAO = cwFilesDBService.loadRecordByParams(conn, "cwf_id = ?::int and cwf_org_id=?::int",
				new SelectParamValue(id), new SelectParamValue(vandOrgId));
		if (fileDAO == null || fileDAO.getCwf_id() == null) {
			throw new SparkBusinessException(new S2Message("common.error.action_not_granted", SparkMessageType.ERROR,
					"User is not authorized to perform this action"));
		}

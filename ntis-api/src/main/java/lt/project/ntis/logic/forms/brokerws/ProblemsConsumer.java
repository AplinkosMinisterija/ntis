...
import lt.project.ntis.logic.forms.brokerws.RcBroker.Problem;

public class ProblemsConsumer implements Consumer<Problem> {
    ...
    private void toBuffer(Problem problem, int index, int total, TextStringBuilder buf) {
        buf.append("  problem ").append(index + 1).append("/").append(total).append(": ").append(problem.message()).append(".");

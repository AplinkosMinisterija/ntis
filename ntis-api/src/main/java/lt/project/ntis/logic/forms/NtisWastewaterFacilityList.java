...
public class NtisWastewaterFacilityList extends FormBase {
    ...
    /**
     * Defines the form and actions for the NTIS Wastewater Facility List form.
     * This includes adding form actions for creating, reading, updating, viewing facility information,
     * approving facilities, and removing them from accounts.
     */
    @Override
    ...
    /**
     *
     * Removes a wastewater treatment facility from the list, setting the wtf_deleted field to true.
     *
     ...
     * @throws NumberFormatException if the ID in the recordIdentifier object cannot be parsed to a double
     * @throws Exception if there is an error executing the UPDATE statement
     */
    public void removeFacility(Connection conn, RecordIdentifier recordIdentifier, Double orgId, Double perId) throws NumberFormatException, Exception {
        ...
        if (queryByOwner) {
            if (orgId != null) {
                wtfId = ntisFacilityOwnersDBService.loadRecordByParams(conn, " WHERE fo.fo_org_id = ?::int AND fo.fo_wtf_id = ?::int GROUP BY fo.fo_wtf_id ",
                        new SelectParamValue(orgId), new SelectParamValue(wtfId)).getFo_wtf_id();
            } else {
                wtfId = ntisFacilityOwnersDBService.loadRecordByParams(conn, " WHERE fo.fo_per_id = ?::int AND fo.fo_wtf_id = ?::int GROUP BY fo.fo_wtf_id ",
    ...
    public void approveFacility(Connection conn, RecordIdentifier recordIdentifier, Double orgId, Double perId) throws NumberFormatException, Exception {
        this.isFormActionAssigned(conn, FormBase.ACTION_UPDATE);
        Double wtfId = recordIdentifier.getIdAsDouble();
        Boolean queryByOwner = (!this.hasUserRole(conn, NtisRolesConstants.MUNICI_SPECIALIST) && !this.hasUserRole(conn, NtisRolesConstants.NTIS_ADMIN)
                && !this.hasUserRole(conn, NtisRolesConstants.INST_WORK) && !this.hasUserRole(conn, NtisRolesConstants.INST_ADMIN)
        ...
        if (queryByOwner) {
            if (orgId != null) {
                wtfId = ntisFacilityOwnersDBService.loadRecordByParams(conn, " WHERE fo_org_id = ?::int AND fo_wtf_id = ?::int limit 1",
                        new SelectParamValue(orgId), new SelectParamValue(wtfId)).getFo_wtf_id();
            } else {
                wtfId = ntisFacilityOwnersDBService.loadRecordByParams(conn, " WHERE fo_per_id = ?::int AND fo_wtf_id = ?::int limit 1",
                        new SelectParamValue(perId), new SelectParamValue(wtfId)).getFo_wtf_id();
            }
        }

        NtisWastewaterTreatmentFaciDAO ntisWastewaterTreatmentFaciDAO = ntisWastewaterTreatmentFaciDBService.loadRecord(conn, wtfId);
     ...
     * @param userId a Double representing the ID of the user making the request
     * @param personId a Double representing the ID of the person associated with the user
     * @param organizationId a Double representing the ID of the organization associated with the user
    ...
    public String getRecList(Connection conn, SelectRequestParams params, Double userId, Double personId, Double organizationId, String language)
            throws Exception {
        ...
        Boolean queryByOwner = (!this.hasUserRole(conn, NtisRolesConstants.MUNICI_SPECIALIST) && !this.hasUserRole(conn, NtisRolesConstants.NTIS_ADMIN)
                && !this.hasUserRole(conn, NtisRolesConstants.INST_ADMIN) && !this.hasUserRole(conn, NtisRolesConstants.INST_WORK)
                && !this.hasUserRole(conn, NtisRolesConstants.VAND_ADMIN) && !this.hasUserRole(conn, NtisRolesConstants.CAR_SPECIALIST)
                && !this.hasUserRole(conn, NtisRolesConstants.PASL_ADMIN));
        ...
        statement.append(" SELECT  "//
                ...
                + "  LEFT JOIN ntis.ntis_address_vw adw ON adw.address_id = ntrs.bn_ad_id  "//
        ...
        stmt.addParam4WherePart("wtf.wtf_facility_latitude::text ", StatementAndParams.PARAM_STRING, advancedParamList.get("wtf_latitude"));
        stmt.addParam4WherePart("wtf.wtf_facility_longitude::text ", StatementAndParams.PARAM_STRING, advancedParamList.get("wtf_longitude"));

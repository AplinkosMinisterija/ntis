...
@Service("CSV_POSTGRE_RESEARCH_ORDERS_LOADER")
public class ExecuteResearchOrdersImportTask extends BaseRequestLogicExecutor {

    private static final Logger log = LoggerFactory.getLogger(ExecuteResearchOrdersImportTask.class);
    ...
    @Autowired
    NtisResearchOrdersImportLoaderJobRequest ntisResearchOrdersImportLoaderJobRequest;
    ...
    public void executeSpecificLogic(Connection conn, String scriptName, SprJobRequestsDAO request) throws Exception {
        ...
        Map<String, String> params = ntisResearchOrdersImportLoaderJobRequest.loadJobRequestParams(conn, request.getJrq_id());
        log.debug("SOURCE_FILE_ID: " + params.get(NtisResearchOrdersImportLoaderJobRequest.SOURCE_FILE_ID));
        SprFilesDAO fileDAO = sprFileDBService.loadRecord(conn, Utils.getDouble(params.get(NtisResearchOrdersImportLoaderJobRequest.SOURCE_FILE_ID)));
        ...
        try (Connection postgreConn = postgreDataSource.getConnection()) {
            ...
            if (params.containsKey(NtisResearchOrdersImportLoaderJobRequest.SOURCE_FILE_ENCODING)) {
                encodingStr = params.get(NtisResearchOrdersImportLoaderJobRequest.SOURCE_FILE_ENCODING);

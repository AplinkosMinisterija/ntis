...
@Service("CSV_POSTGRE_TECH_ORDERS_LOADER")
public class ExecuteTechOrdersImportTask extends BaseRequestLogicExecutor {

    private static final Logger log = LoggerFactory.getLogger(ExecuteTechOrdersImportTask.class);
    ...
    @Autowired
    NtisTechOrdersImportLoaderJobRequest ntisTechOrdersImportLoaderJobRequest;
    ...
    public void executeSpecificLogic(Connection conn, String scriptName, SprJobRequestsDAO request) throws Exception {
        ...
        Map<String, String> params = ntisTechOrdersImportLoaderJobRequest.loadJobRequestParams(conn, request.getJrq_id());
        log.debug("SOURCE_FILE_ID: " + params.get(NtisTechOrdersImportLoaderJobRequest.SOURCE_FILE_ID));
        SprFilesDAO fileDAO = sprFileDBService.loadRecord(conn, Utils.getDouble(params.get(NtisTechOrdersImportLoaderJobRequest.SOURCE_FILE_ID)));
        ...
        try (Connection postgreConn = postgreDataSource.getConnection()) {
            ...
            if (params.containsKey(NtisTechOrdersImportLoaderJobRequest.SOURCE_FILE_ENCODING)) {
                encodingStr = params.get(NtisTechOrdersImportLoaderJobRequest.SOURCE_FILE_ENCODING);

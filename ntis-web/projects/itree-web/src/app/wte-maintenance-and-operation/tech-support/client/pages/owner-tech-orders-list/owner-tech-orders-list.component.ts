...
import { NTIS_OWNER_TECH_ORDERS_LIST } from '@itree-web/src/app/ntis-shared/constants/forms.const';
...
import { OwnerTechOrdersBrowseRow, SelectOption } from '../../../models/browse-pages';
...
import { PRIEZIURA } from '@itree-web/src/app/ntis-shared/constants/classifiers.const';
...
@Component({
  selector: 'app-owner-tech-orders-list',
  templateUrl: './owner-tech-orders-list.component.html',
  styleUrls: ['./owner-tech-orders-list.component.scss'],
  standalone: true,
  imports: [
    CommonModule,
    ItreeCommonsModule,
...
export class OwnerTechOrdersListComponent extends BaseBrowseForm<OwnerTechOrdersBrowseRow> implements OnInit {
  readonly formCode = NTIS_OWNER_TECH_ORDERS_LIST;
  ...
  readonly translationsReference = 'wteMaintenanceAndOperation.techSupport.client.pages.ownerTechOrdersList';
  reviewInfo: NtisReviewsDAO = {} as NtisReviewsDAO;
  showReviewDialog: boolean = false;
  exportData: OwnerTechOrdersBrowseRow[] = [];
  ...
  protected load(
    ...
    if (this.loaded) {
      ...
      this.techService
        .getOwnerTechOrders(pagingParams, params, extendedParams)
        .pipe(
          map((result) => {
  ...
  getNtisActions(row: OwnerTechOrdersBrowseRow): NtisTableRowActionsItem[] {
